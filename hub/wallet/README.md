---
description: >-
  Alby Hub is a wallet app that runs a full lightning node, which can be
  accessed by various apps, and it also has a web interface for sending and
  receiving satoshis over Lightning Network.
---

# 👛 Wallet

### Introduction

[Alby Hub](https://getalby.com/alby-hub) is available everywhere where you want pay with bitcoin: on the web, in apps or on mobile. But of course you can also send and receive bitcoin directly from the Alby Hub itself. <br>

<figure><img src="../.gitbook/assets/chrome_jSGiOGoSXi.png" alt=""><figcaption><p>Alby Hub wallet</p></figcaption></figure>



{% content-ref url="send.md" %}
[send.md](send.md)
{% endcontent-ref %}

{% content-ref url="receive.md" %}
[receive.md](receive.md)
{% endcontent-ref %}

{% content-ref url="buy-bitcoin.md" %}
[buy-bitcoin.md](buy-bitcoin.md)
{% endcontent-ref %}





***

_Alby Hub was created with a lot of effort, love, and the vision of a better world with Bitcoin. We’re grateful for your consideration in getting a_ [_membership_ ](../alby-hub-flavors/alby-cloud.md)_and supporting our work._

_Best regards, Your Alby Team 💛🐝_
